...
def createFlowRule(notif):
    # Crea la regla que hay que eliminaer en el switch (en el idioma del switch) 
...
# def buildPacketOutMetadata(p4info_obj_map, name="packet_out", **kwargs):
#     """
#     Genera la lista de metadatos para PacketOut de forma DINÁMICA.
#     Busca los nombres en el P4Info, averigua su ID y ancho de bits,       Mierdon de softcoding tbh
#     y codifica los valores automáticamente.
    
#     Uso: buildPacketOutMetadata(mapa, opcode=1, reserved1=0, operand0=2)
#     """
    
#     # 1. REUTILIZAMOS TU FUNCIÓN
#     # Esto nos devuelve algo como: {1: {'name': 'opcode', 'bitwidth': 8}, ...}
#     meta_info_dict = controllerPacketMetadataDictKeyId(p4info_obj_map, name)
    
#     metadata_list = []
    
#     # 2. Recorremos el diccionario que nos devolvió tu función
#     # Usamos .values() porque queremos ver los objetos internos ({'name':...})
#     for info in meta_info_dict.values():
#         name = info['name']
        
#         # 3. Verificamos si este campo está en los argumentos que pasaste (kwargs)
#         if name in kwargs:
#             val_int = kwargs[name]
#             bitwidth = info['bitwidth']
#             id_num = info['id']
            
#             # 4. Convertimos a bytes (La parte "hard" que antes hacías a ojo)
#             # Calculamos bytes necesarios: (bits + 7) // 8
#             nbytes = (bitwidth + 7) // 8
#             val_bytes = val_int.to_bytes(nbytes, byteorder='big')
            
#             # 5. Añadimos a la lista final
        ...
        if len(payload) > 0:
            ...
            if pktinfo['metadata']['punt_reason'] == global_data['punt_reason_name2int']['FLOW_UNKNOWN']:
                ...
                metadatas = packetOutMetadataList(
                    ...
                    0, dest_port_int)
                # metadatas = buildPacketOutMetadata(global_data["p4info_obj_map"],
                #                                    opcode = global_data['controller_opcode_name2int']['SEND_TO_PORT_IN_OPERAND0'],
                #                                    reserved1 = 0,
                #                                    operand0=dest_port_int)
        ...
        while True:
            ...
            if notif["type"] == "packet-in":
                ...
                if notif["sw"].name not in notif_db:
                    ...
                    cleanExpiredNotification(notif["sw"].name, 10)

                table_entry = createFlowRule(notif)

                if not checkFlowRule(notif["sw"].name, table_entry):    # check da true si ya me ha llegado la notif, si no me habia llegado
                    addNotification(notif["sw"].name, table_entry)      # tengo que añadir la notif a la db y luego
                    deleteFlowRule(notif["sw"], table_entry)            # eliminar la regla del switch (idle_notif por timeout bro, espabila)
                else:
...
async def main(p4info_file_path, bmv2_file_path):
    ...
    try:
        ...
        await asyncio.gather(pkt_s1, pkt_s2, idle_notif_s1, idle_notif_s2, proc_notif)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--p4info', help='p4info proto in text format from p4c',
                        ...
                        default='./build/first.p4.p4info.txtpb')
    parser.add_argument('--bmv2-json', help='BMv2 JSON file from p4c',
                        type=str, action="store", required=False,
                        default='./build/first.json')
