...
const int DIGEST_LEARNING = 22;
const bit<4> TRIGGER_RESUBMIT = 3;
...
control MyIngress(inout headers_t hdr,
    ...
    action flooding (bit<8> mcast) {
        ...
        digest<mac_learning_digest_t>(DIGEST_LEARNING, { 
    ...
    action flow_unknown () {
        ...
        if (meta.resubmitted < 1) {
            ...
            // Marcamos el metadato para la siguiente vuelta
            meta.resubmitted = meta.resubmitted + 1;
            
            // Re-enviamos el paquete al inicio
            // El '0' es el índice de una lista de campos si quieres preservar algo más
            //resubmit_preserving_field_list(FL_RESUBMIT);
        }
        else{
            drop_packet();
        }
    }
    action marking (bit<8> key) 
    {
        meta.key_tunnel = key;
    }
    ...
    apply {
        ...
        if (hdr.packet_out.isValid()) {
            ...
            if(hdr.ethernet.dstAddr == BROADCAST_ADDR){
