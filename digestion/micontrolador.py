...
def processBroadcast(message):
    ...
    payload = message["payload"].data
    print("Received digestIn broadcast learning from switch %s"
            "" % (len(payload), message["sw"].name))
    # 3. Recorremos los datos decodificados
    for entry in decoded_data:
        srcport = entry['ingress_port']
    ...
    try:
        ...
        for sw in switches_info:
            ...
            try:
                ...
                conn.EnableDigest(global_data["id_mac_learning"], max_timeout_ns=global_data["timeout"], max_list_size=100)

                global_data["id_digest_unknown"] = p4info_helper[prog].get_digests_id("digest_t")
                conn.EnableDigest(global_data["id_digest_unknown"], max_timeout_ns=global_data["timeout"], max_list_size=100)
