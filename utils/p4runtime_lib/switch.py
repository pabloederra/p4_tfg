...
class StreamDispatcher:
    def __init__(self, stream):
        ...
        self.digest_queue = Queue() # Nuevo DIGEST
    ...
    def _dispatch_loop(self):
        for msg in self.stream:
            ...
            if msg.HasField("arbitration"):
                ...
                self.digest_queue.put(msg.digest) # Nuevo DIGEST
                timestamp = datetime.now().strftime("%H:%M:%S.%f")[:-3]
                print("TS PUT:", timestamp)
...
class SwitchConnection(object):
    ...
    # Nuevo DIGEST
    def DigestIn(self, dry_run=False):
        # Lee de la nueva cola del dispatcher
        request = self.dispatcher.digest_queue.get()
        if dry_run:
            print("P4 Runtime DigestIn: ", request)
        else:
            return request
    
    # Pa mandar los ACKs
    def DigestAck(self, digest_id, list_id):
        request = p4runtime_pb2.StreamMessageRequest()
        request.digest_ack.digest_id = digest_id
        request.digest_ack.list_id = list_id
        self.requests_stream.put(request)

    def EnableDigest(self, digest_id, max_timeout_ns=10000000, max_list_size=100):
        """
        Activa el Digest en el switch. 
        max_timeout_ns: tiempo max de espera para agrupar (10ms por defecto)
        max_list_size: cuántos mensajes agrupar antes de enviar al controlador
        """
        # 1. Buscar ID del digest (asumiendo que tienes acceso a p4info_helper)
        # Si no lo tienes directo, podrías pasar el ID como argumento
        #digest_id = self.p4info_helper.get_digests_id(digest_name)
        ...
        digest_entry.config.ack_timeout_ns = 0 # 0 significa no requiere ACK explícito en algunas versiones
        
        self.client_stub.Write(request)
        print(f"Digest con ID: {digest_id} habilitado.")
