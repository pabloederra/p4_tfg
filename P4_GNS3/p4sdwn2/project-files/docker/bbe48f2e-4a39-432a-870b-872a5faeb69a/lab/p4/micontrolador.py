...
def get_p4_config_from_topo(topo_file='./swtopo.json'):
...
def addMarkedRule(sw, eth_addr, srcport, tunnel_id, prog):
   ...
   # 1. Buscamos la tabla iterando directamente sobre el p4info
    ...
    if eth_match_field == None:
        print(f"El switch no tiene el match_field: {sw.name}")
        return
...
async def main(switches_config):
    ...
    try:
        ...
        for sw in switches_info:
            ...
            # Master arbitration
            print(f"Solicitando Mastership para {sw['name']}...")
            #conn.MasterArbitrationUpdate()
            conn.MasterArbitrationUpdate(election_id=1)
            # 2. ESPERA CRUCIAL
            # En entornos de red virtual (GNS3/Docker), el handshake gRPC tarda unos milisegundos.
            time.sleep(2) 

            # Instalar el P4 program
            print(f"Instalando Pipeline en {sw['name']}...")
            conn.SetForwardingPipelineConfig(
                p4info=p4info_helper[switches_config[sw['name']]['program_name']].p4info,
                bmv2_json_file_path=switches_config[sw['name']]['bmv2_json']
            )
            print(f"Installed P4 {switches_config[sw['name']]['program_name']} using SetForwardingPipelineConfig on {sw['name']}")
