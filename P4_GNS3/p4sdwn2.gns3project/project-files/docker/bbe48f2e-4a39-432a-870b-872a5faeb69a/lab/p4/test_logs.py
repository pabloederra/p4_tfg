...
def processPacket(message, prog):
    ...
    if reason_name == 'FLOW_UNKNOWN':
        ...
        elif pkt.dst in tabla_actual:
            dst_port = tabla_actual[pkt.dst]
            tunnel_id = pktinfo['metadata']['key_tunnel']
            logger.info(f"║ [FLOW] Instalando regla: {pkt.src} -> {pkt.dst} (Port {dst_port})")
...
def main(switches_config):
    ...
    try:
        ...
        for sw in switches_info:
            ...
            logger.debug(f"Installed P4 {switches_config[sw['name']]['program_name']} using SetForwardingPipelineConfig on {sw['name']}")
