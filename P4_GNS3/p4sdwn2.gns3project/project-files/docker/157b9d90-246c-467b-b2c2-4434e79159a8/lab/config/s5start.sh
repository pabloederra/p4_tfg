...
ip link add gretap141 type gretap \
    local 192.168.1.5 remote 192.168.1.4 \
    key 1001
...
for intf in gretap141 gretap143; do # añade aquí tus interfaces
...
simple_switch_grpc --device-id 0 \
  ...
  -i 141@gretap141 \
  -i 143@gretap143 \
  --log-file ../logs/s5.log \
  --no-p4 \
  -- \
  --grpc-server-addr 0.0.0.0:9559 \
  --cpu-port 510 > /dev/null 2>&1 &

echo "Switch ejecutándose en segundo plano. Logs en logs/s5.log"
# sleep infinity
